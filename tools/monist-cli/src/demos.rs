...
pub fn run_holographic_demo() {
    println!("{}", "\n========================================================".cyan());
    println!("{}", "  HOLOGRAPHIC SIEVE VISUALIZER: O(1) EXCLUSION ROUTING  ".cyan().bold());
    println!("{}", "========================================================\n".cyan());
    ...
    println!("Swarm initialized with {} active nodes.", total_nodes.to_string().cyan());
    ...
    pb_standard.abandon_with_message("O(N) Traversal Abandoned - Initiating Monist Protocol".yellow().to_string());
    
    thread::sleep(Duration::from_millis(1000));
    
    println!("\n{}", ">>> ENGAGING TOPOLOGICAL EXCLUSION FIRST ROUTING <<<".magenta().bold());
    println!("{}\n", "Mapping logical contradictions as negative-weight cycles...".magenta());
    thread::sleep(Duration::from_millis(1200));

    // Collapse
    let pb_fast = m.add(ProgressBar::new(total_nodes));
    let collapse_sty = ProgressStyle::with_template(
        "[{elapsed_precise}] {bar:40.red/red} {pos:>7}/{len:7} {msg}",
    )
    .unwrap()
    .progress_chars("X#-");
    ...
    println!("\n{}", "================ METRICS ================".cyan().bold());
    println!("Theoretical O(N) Comparisons: {}", total_nodes.to_string().yellow());
...
pub fn run_agentic_demo() {
    println!("{}", "\n========================================================".cyan());
    println!("{}", "   AGENTIC REFLECTION: LEAST SYNTACTIC ACTION ROUTING   ".cyan().bold());
    println!("{}", "========================================================\n".cyan());
    ...
    let sty_well_founded = ProgressStyle::with_template(
        "[{elapsed_precise}] {prefix:>20} {bar:40.green/green} {msg}",
    )
    .unwrap()
    .progress_chars("##-");
    ...
    for i in 0..=k_limit {
        ...
        if i > 10 && i <= 18 {
            ...
            pb_safe.finish_with_message(format!("{}", "PATH SELECTED (Least Action)".green().bold()));
            break;
        }
    ...
    println!("\n{}", ">>> STRUCTURAL SNAP: NON-WELL-FOUNDED PATH ABORTED <<<".magenta().bold());
    thread::sleep(Duration::from_millis(500));

    println!("\n{}", "================ METRICS ================".cyan().bold());
    println!("Live Bellman-Ford cycle mean (μ*): {}", "-2.9".red());
    println!("K-Iteration Depth:                 {}/{}", k_limit.to_string().yellow(), k_limit.to_string().yellow());
