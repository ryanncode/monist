...
fn main() {
    ...
    println!(
        "By explicitly equating the subset to its element via the SC mapping (subset = elem),"
    );
    ...
    pb_sc.finish_with_message(
        "[OK] SCC algorithm successfully recognized and collapsed the SC island natively.\n",
    );
