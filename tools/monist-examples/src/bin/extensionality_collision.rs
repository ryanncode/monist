    ...
    match session.eval(formula, "base_extensionality_collision") {
        Ok(dist) => {
            ...
            for (var, idx) in &session.graph.var_to_idx {
                match var.0 {
                    monist_core::ast::Var::Free(ref name) => {
                        if name == "K" { k_idx = Some(*idx); }
                        if name == "Q" { q_idx = Some(*idx); }
                        if name == "x" { x_idx = Some(*idx); }
                        if name == "a" { a_idx = Some(*idx); }
                    }
                    _ => {}
                }
            }
            ...
            if let (Some(k), Some(q), Some(x), Some(a)) = (k_idx, q_idx, x_idx, a_idx) {
                ...
                assert_eq!(dist_k, dist_q, "Extensionality Equivalence requires K and Q to have same typestate limits");
                assert!(dist_k <= dist_x + 2, "Kuratowski offset guarantees K is at most x + 2");
                assert_eq!(dist_q, dist_a, "Quine geometric map guarantees Q preserves exact typestate of a");
                
                println!("[SUCCESS] Extensionality Collision topological weights correctly verified!");
            }
        }
        Err(e) => {
            panic!("Test Failed! Expected success without paradox halt, but got Error: {}", e);
        }
    }

    println!("\n--- Advanced Deep Hierarchy Verification ---");
    println!("Testing Deep Nesting: Kuratowski K3 = K(K(x,y), K(z,w)) vs Quine Q3 = Q(Q(a,b), Q(c,d))");
    ...
    match session2.eval(deep_formula, "deep_extensionality_collision") {
        Ok(dist) => {
            ...
            for (var, idx) in &session2.graph.var_to_idx {
                match var.0 {
                    monist_core::ast::Var::Free(ref name) => {
                        if name == "K3" { k3_idx = Some(*idx); }
                        if name == "Q3" { q3_idx = Some(*idx); }
                        if name == "x" { x_idx = Some(*idx); }
                        if name == "a" { a_idx = Some(*idx); }
                    }
                    _ => {}
            ...
            if let (Some(k3), Some(q3), Some(x), Some(a)) = (k3_idx, q3_idx, x_idx, a_idx) {
                ...
                assert_eq!(dist_k3, dist_q3, "Extensionality Equivalence maintained at boundary K3=Q3");
                assert!(dist_k3 <= dist_x + 4, "Deep Kuratowski scaling properly bounds type extraction at +4 limits");
                assert_eq!(dist_q3, dist_a, "Deep Quine geometric map maintains stable 0 typestate cascade");
