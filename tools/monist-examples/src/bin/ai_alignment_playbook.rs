...
use monist_core::ast::{Var, Atomic, Formula, FormulaArena};
...
fn main() {
    ...
    let eq_reward = f_arena.add(Formula::Atom(Atomic::Eq(reward_1.clone(), reward_2.clone())));
    
    // 3. To prove it's robust, we map infinite recursive layers (simulated by equating layer 2 to N)
    let eq_reward_inf = f_arena.add(Formula::Atom(Atomic::Eq(reward_2.clone(), reward_n.clone())));
    ...
    let constraints = extract_constraints_aux(&f_arena, main_f, 0);
    ...
    pb_bellman.set_message("Verifying topological stability (Bellman-Ford Extensionality Check)...");
    ...
    println!("\n[SC-CUT WITNESS]");
    println!("Generating SMT-LIB Witness. Notice how the recursively deep Reward_Reward_AgentState");
    ...
    println!("--------------------------------------------------------------------------------");
    let smt_out = export_smt_lib(&graph, "RecursiveRewardStability_NonWellFounded");
    println!("{}\n", smt_out);
