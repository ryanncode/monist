...
## 6. Mathematical Diagnostic Binaries & Visual Demos

The workspace provides ready-to-run diagnostic binaries in `tools/monist-examples` that test edge-case paradoxes, transfinite limits, and continuous-to-discrete pipelines:

### Set-Theoretic Paradox Diagnostics
```bash
# Specker's Refutation of Global Choice across disjoint integer elevations
cargo run -p monist-examples --bin specker_refutation

# Extensionality Collision detection (Kuratowski vs Quine pairs)
cargo run -p monist-examples --bin extensionality_collision

# Russell's Paradox K-Iteration halting verification
cargo run -p monist-examples --bin russell

# Strongly Cantorian bedrock and Quine atoms
cargo run -p monist-examples --bin strongly_cantorian

# Transfinite cardinal arithmetic without the Axiom of Choice
cargo run -p monist-examples --bin transfinite
```

### Canonical ITP Tactics Example Suites
```bash
# Strategic Cut tactics and intermediate lemmas
cargo run --example strategic_cut

# Monotonicity proofs across power set topologies
cargo run --example monotonicity_powerset

# Equivalence and DAG isomorphism via reflexivity
