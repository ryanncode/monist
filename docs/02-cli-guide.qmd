...
## Logic Parsing & Syntax Cheatsheet
...
* **Set Theory Operations:**
  ...
  * Set Comprehension: `{x | P(x)}` (e.g., `{x | ~(x e x)}`)
...
* `have <name> <formula>`: Prove a sub-goal and add it to the local context.
...
### Topological & Advanced Tactics

* `stratify`: Weak stratification topological check via Bellman-Ford shortest-path routing.
* `refl`: DAG topological equivalence check via SCC isomorphism.
* `schonfinkel`: Compiles goal or formula into pure SKI combinator tree representation.
* `step [formula]`: Executes geometric evaluation on the active target goal or a specific formula, providing real-time color-coded feedback on topological bounds.
* `simp`: Normalizes formulas into Disjunctive Normal Form (DNF) using De Morgan push-negation laws.
* `rw <hyp>` / `rewrite <hyp>`: Substitutes variables in the target AST using an active equality hypothesis.
* `elevate [name]`: Applies Forster's $T$-operator ($x \mapsto x_\iota$) to elevate variables across topological stratum boundaries.
* `collapse_loop`: Contracts 0-weight equality rings into unified concept vertices mid-proof.
* `sc_cut <set>`: Injects Strongly Cantorian bedrock axioms into the local context.

---

## Troubleshooting Paradoxes

:::{.callout-warning title="Handling Extensionality Collisions"}
...
* **Universal Set Boundaries:** If you are trying to map functions across the boundary of the Universal Set ($V$), you must step *through* the topological threshold. Attempting to bridge distinct integer typestate elevations without a stabilizer will cause the Bellman-Ford cycle detector to terminate your proof. You must explicitly invoke the $T$-operator ($x \mapsto \iota"x$) or rely on the engine's automated `T-Functor Synthesis` to gracefully absorb the topological friction.
* **Saturated Loops:** If you are stuck in an infinite recursion, check if you accidentally defined an unstratified loop outside of a Strongly Cantorian boundary. Use the `step` command to visualize the $K$-Iteration depth bounds in real-time, observing exactly where the loop fails to terminate.
:::
...
* **Set Comprehension (`{x | P(x)}`)**: Generates a **`+1` weight** constraint boundary. The internal formula $P(x)$ is forced to exist one topological level lower than the set defined by the comprehension. If a $+1$ shift topological cycle spans *across* this boundary (i.e. `in_comp == true`), it is recognized as a mathematical paradox and strictly rejected, whereas free-floating cycles outside of comprehensions may be isolated as Strongly Cantorian bedrock.
...
```bash
cargo run -p monist-cli -- export-smt "x e y /\ y e z /\ z e x" > trace.smt2
```

---

## Under the Hood: REPL Tactic Integration
...
When you query for a diagnostic limit (`eval` or `step`), the engine collapses 0-weight sub-graphs (equalities) using Kosaraju's SCC algorithm and tests the geometry:
